...
4. **If creating new issue:**
   ...
   - Plan file is at `.issues/BOX-N/plan.md`
   - Fill in issue.md with requirements and acceptance criteria

5. **Confirm tracking:**
   - Display: "Working on BOX-N: <title>"
   - Display: "Remember to run `/issue session BOX-N` after completing work"
...
4. **Checklist output:**
   ```
   Workflow Check:
   [x] Issue exists: BOX-18
   [x] Status: in-progress
   [x] Plan exists: yes
   [ ] Session summary current: NO - 2 commits since summary-2.md

   Action needed: Run `/issue session BOX-18`
   ```
...
4. **Confirm completion:**
   - Display: "Session summaries created for: BOX-N, BOX-M"
...
2. If found: `/issue work BOX-N`
3. If not: `/issue create "Fix: bug description"`
4. Implement fix
5. `/issue session BOX-N` - record the fix
6. If complete: `/issue done BOX-N`
...
5. `/issue session BOX-N` after each work block
6. `/issue done BOX-N` when feature complete

### "Resuming work from previous session"
1. Check `.issues/index.md` for in-progress issues
2. `/issue work BOX-N` to see context
3. Read latest `summary-N.md` for where we left off
4. Continue implementation
5. `/issue session BOX-N` before ending
...
4. `/issue session BOX-N` to record creation
...
- **Making changes without an active issue** - Always have a BOX-N
